...
| 高风险：执行完整/混淆 JS | 默认关闭；仅在静态解析失败且完全信任脚本时启用 |
| 优先音质 | 如 `flac` / `320k` |
...
重新配置菜单仅用于修改：用户名、密码、自定义音源 JS 链接和高风险混淆 JS 开关。

### 高风险混淆 JS 模式

默认关闭。普通明文脚本会继续使用静态解析，不执行 JavaScript。只有静态解析失败且该开关开启时，集成才会在受限 QuickJS 沙箱中执行完整 LX 插件。

沙箱不暴露 Home Assistant、文件系统、系统进程或环境变量，但会向脚本开放受控的 HTTP(S) 网络请求，以满足 LX 插件工作需要。恶意或被篡改的脚本仍可能：

- 向外部公网服务器发送歌曲 ID、来源、音质等请求数据（本集成会拦截私网、本地和保留地址）
- 消耗 CPU、内存或网络资源
- 返回恶意、不安全或未经授权的播放地址

**请仅对完全信任且有权使用的脚本启用。启用即表示使用者已理解上述风险，并自愿承担由此产生的全部后果。**
